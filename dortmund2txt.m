datasets = strvcat('combine');

for ith_data = 1: size(datasets, 1)
    dataset = strcat(datasets(ith_data, :));
    path2data = ['C:\Users\samgostar\Desktop\thises\package\V2\test\vg\no lable\'];    % path to folder containing data
    ...
    fid = fopen(sprintf('%s.txt', dataset), 'w');
    fprintf(fid, '%d\n', num_graphs);
